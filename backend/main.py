...
class ChatResponse(BaseModel):
    ...
    # Automatically generated after a successful CareAI result.
...
def chat(request: ChatRequest):
    ...
    # ========================================================
    # AUTO_GEMINI_RESULT_EXPLANATION
    # ========================================================
    #
    # Disease suggestion has ALREADY been generated by
    # CareAI's Multinomial NB model.
    #
    # Gemini receives ONLY the completed CareAI result
    # and converts it into a short natural explanation.
    #
    # Gemini does NOT calculate disease probabilities here.

    auto_explanation = None
    auto_explanation_source = None

    if (
        request.message
        and result.get("status") == "success"
        and result.get("top_predictions")
    ):
        ...
        if detected_language == "bangla":
            explanation_language = "bangla"

        elif detected_language in {
            "banglish",
            "mixed",
        }:
            explanation_language = "banglish"
        ...
        if gemini_symptom_engine is not None:
            ...
            if auto_explanation:
                auto_explanation_source = "gemini"

        # ----------------------------------------------------
        # Gemini quota/error fallback
        # ----------------------------------------------------
        #
        # Result will still contain an explanation even if
        # Gemini explanation request fails.
        ...
        if not auto_explanation:
            ...
            auto_explanation_source = "local_fallback"
